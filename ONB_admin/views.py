...
from ONB_admin.form import (CreateNotificationForm, AddCommentForm)
...
class DashboardView(View):
    def get(self, request):
        users = Accounts.objects.filter(is_staff=False).count()
        posts = Notification.objects.all().count()
        context = {
            'users':users,
            'posts':posts,
        }
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            try:
                ...
                form.save()
                messages.success(request, f'Post has been published!')
                return redirect('super:notify')
            except Accounts.DoesNotExist:
                messages.error(request, 'User does not exist!!')
                return redirect(to="auth:login")

        messages.error(request, f'Error uploading Post\n {form.errors} ')
...
class EditNotificationView(View):
    ...
    def post(self, request, notification_id):
        try:
            notification = Notification.objects.get(id=notification_id)
            form = CreateNotificationForm(request.POST, request.FILES, instance=notification)
...
class SearchNotificationView(View):
    def get(self, request):
        ...
        return render(request, 'basics/search-result.html', context)
